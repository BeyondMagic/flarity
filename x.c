...
static void xloadfonts(char *, char *, char *, char *, double, double, double, double);
...
static double usedfontsize = 0, usedfontsize_italics = 0, usedfontsize_bold = 0, usedfontsize_roman = 0;
static double defaultfontsize = 0, defaultfontsize_italics = 0, defaultfontsize_bold = 0, defaultfontsize_roman = 0;
...
zoom(const Arg *arg)
{
	...
	larg.fi = usedfontsize_italics + arg->f;
...
zoomreset(const Arg *arg)
{
	...
	if (defaultfontsize > 0 && defaultfontsize_italics > 0) {
		larg.f = defaultfontsize;
		larg.fi = defaultfontsize_italics;
		zoomabs(&larg);
	}
}
...
//  1. Main function is "xloadfonts".
//  2. The macros are used to minify code as always.

	void
xloadfonts (char *fontstr, char*fontstr_italics, char*fontstr_bold, char*fontstr_roman,
		double fontsize, double fontsize_italics, double fontsize_bold, double fontsize_roman)
{

	// 1. Font patterns to be defined by the fonts given.
	FcPattern *pattern;
	FcPattern *pattern_italics;
	FcPattern *pattern_bold;
	FcPattern *pattern_roman;

	// 2. Font values to be defined by the fonts given.
	double fontval;
	double fontval_italics;
	double fontval_bold;
	...
	*/
	...
	// 3. Default font initializing pattern.
	{
		...
		// II. Italics font.
		if (fontstr_italics[0] == '-')
			pattern_italics = XftXlfdParse(fontstr_italics, False, False);
		else
			pattern_italics = FcNameParse((FcChar8 *)fontstr_italics);

		// III. Bold font.
		if (fontstr_bold[0] == '-')
			pattern_bold = XftXlfdParse(fontstr_bold, False, False);
		else
			pattern_bold = FcNameParse((FcChar8 *)fontstr_bold);

		// III. Roman font.
		if (fontstr_roman[0] == '-')
			pattern_roman = XftXlfdParse(fontstr_roman, False, False);
		else
			pattern_roman = FcNameParse((FcChar8 *)fontstr_roman);

	}

	// 4. Handle pattern definition.
	{

		// I. Default font.
		if (!pattern)
			die("[xloadfonts] can't open font \"%s\"\n", fontstr);

		// II. Italics font.
		if (!pattern_italics)
			die("[xloadfonts] Can't open italics font \"%s\"\n", fontstr_italics);

		// II. Bold font.
		if (!pattern_bold)
			die("[xloadfonts] Can't open bold font \"%s\"\n", fontstr_bold);

		// II. Roman font.
		if (!pattern_roman)
			die("[xloadfonts] Can't open roman font \"%s\"\n", fontstr_roman);

	}

	// 5. Handle font size given on the name.
	{
		...
		if (fontsize > 1) {
			...
			// B. Set font size as the default given here.
			...
			if (FcPatternGetDouble(pattern, FC_PIXEL_SIZE, 0, &fontval) == FcResultMatch)
				...
				// 1. Default font size is 12, if none given. This is to have a known usedfontsize value.
		...
		// II. Italics font.
		if (fontsize_italics > 1) {

			// A. Load pixel size.
			FcPatternDel(pattern_italics, FC_PIXEL_SIZE);
			FcPatternDel(pattern_italics, FC_SIZE);
			FcPatternAddDouble(pattern_italics, FC_PIXEL_SIZE, (double)fontsize_italics);

			// B. Set font size as the default given here.
			usedfontsize_italics = fontsize_italics;

		} else {

			if (FcPatternGetDouble(pattern_italics, FC_PIXEL_SIZE, 0, &fontval_italics) == FcResultMatch)
				usedfontsize_italics = fontval_italics;
			else if (FcPatternGetDouble(pattern_italics, FC_SIZE, 0, &fontval_italics) == FcResultMatch)
				usedfontsize_italics = -1;

			else {

				// 1. Default font size is 12, if none given. This is to have a known usedfontsize_italics value.
				FcPatternAddDouble(pattern_italics, FC_PIXEL_SIZE, 12);

				usedfontsize_italics = 12;

			}

			defaultfontsize_italics = usedfontsize_italics;
		}
		...
		if (fontsize_bold > 1) {
			...
			// A. Load pixel size.
			FcPatternDel(pattern_bold, FC_PIXEL_SIZE);
			FcPatternDel(pattern_bold, FC_SIZE);
			FcPatternAddDouble(pattern_bold, FC_PIXEL_SIZE, (double)fontsize_bold);

			// B. Set font size as the default given here.
			usedfontsize_bold = fontsize_bold;

		} else {

			if (FcPatternGetDouble(pattern_bold, FC_PIXEL_SIZE, 0, &fontval_bold) == FcResultMatch)
				usedfontsize_bold = fontval_bold;
			else if (FcPatternGetDouble(pattern_bold, FC_SIZE, 0, &fontval_bold) == FcResultMatch)
				usedfontsize_bold = -1;

			else {

				// 1. Default font size is 12, if none given. This is to have a known usedfontsize_bold value.
				FcPatternAddDouble(pattern_bold, FC_PIXEL_SIZE, 12);

				usedfontsize_bold = 12;

			}

			defaultfontsize_bold = usedfontsize_bold;
		}
		...
		if (fontsize_roman > 1) {
			...
			// A. Load pixel size.
			FcPatternDel(pattern_roman, FC_PIXEL_SIZE);
			FcPatternDel(pattern_roman, FC_SIZE);
			FcPatternAddDouble(pattern_roman, FC_PIXEL_SIZE, (double)fontsize_roman);

			// B. Set font size as the default given here.
			usedfontsize_roman = fontsize_roman;

		} else {

			if (FcPatternGetDouble(pattern_roman, FC_PIXEL_SIZE, 0, &fontval_roman) == FcResultMatch)
				usedfontsize_roman = fontval_roman;
			else if (FcPatternGetDouble(pattern_roman, FC_SIZE, 0, &fontval_roman) == FcResultMatch)
				usedfontsize_roman = -1;

			else {

				// 1. Default font size is 12, if none given. This is to have a known usedfontsize_roman value.
				FcPatternAddDouble(pattern_roman, FC_PIXEL_SIZE, 12);

				usedfontsize_roman = 12;

			}

			defaultfontsize_roman = usedfontsize_roman;
		}

	}

	// 5. Verify if we could load all fonts in X.
	{
		...
		// II. Italics font.
		if (xloadfont(&dc.ifont, pattern_italics))
			die("can't open font %s\n", fontstr_italics);

		// II. Bold font.
		if (xloadfont(&dc.ibfont, pattern_bold))
			die("can't open font %s\n", fontstr_bold);

		// II. Roman font.
		if (xloadfont(&dc.bfont, pattern_roman))
			die("can't open font %s\n", fontstr_roman);

	}

	// 6. Handle negative sizes for fonts before it tries to load.
	{
		...
		// II. Italics font.
		if (usedfontsize_italics < 0) {

			FcPatternGetDouble(dc.ifont.match->pattern,
					FC_PIXEL_SIZE, 0, &fontval_italics);
			usedfontsize_italics = fontval_italics;
			if (fontsize_italics == 0)
				defaultfontsize_italics = fontval_italics;

		}

		// III. Bold font.
		if (usedfontsize_bold < 0) {

			FcPatternGetDouble(dc.ibfont.match->pattern,
					FC_PIXEL_SIZE, 0, &fontval_bold);
			usedfontsize_bold = fontval_bold;
			if (fontsize_bold == 0)
				defaultfontsize_bold = fontval_bold;

		}

		// IV. Roman font.
		if (usedfontsize_roman < 0) {

			FcPatternGetDouble(dc.bfont.match->pattern,
					FC_PIXEL_SIZE, 0, &fontval_roman);
			usedfontsize_roman = fontval_roman;
			if (fontsize_roman == 0)
				defaultfontsize_roman = fontval_roman;

		}

	}

	// 7. Setting character width and height basing on the default font.
	{
	...
	// 8. Open default fonts into the terminal.
	{

		// Load Italics font.
		FcPatternDel(pattern_italics, FC_SLANT);
		FcPatternAddInteger(pattern_italics, FC_SLANT, FC_SLANT_ITALIC);
		if (xloadfont(&dc.ifont, pattern_italics))
			die("[xloadfont] can't open italics font %s\n", fontstr_italics);

		// Load Bold font.
		FcPatternDel(pattern_bold, FC_WEIGHT);
		FcPatternAddInteger(pattern_bold, FC_WEIGHT, FC_WEIGHT_BOLD);
		if (xloadfont(&dc.ibfont, pattern_bold))
			die("can't open bold font %s\n", fontstr_bold);

		// C. Load Slant font.
		FcPatternDel(pattern_roman, FC_SLANT);
		FcPatternAddInteger(pattern_roman, FC_SLANT, FC_SLANT_ROMAN);
		if (xloadfont(&dc.bfont, pattern_roman))
			die("can't open roman font %s\n", fontstr_roman);

		// D. Free patterns from memory.
		{
			FcPatternDestroy(pattern);
			FcPatternDestroy(pattern_italics);
			FcPatternDestroy(pattern_bold);
			FcPatternDestroy(pattern_roman);
		}
	}
}
...
xrdb_load(void)
{
	...
	if (xrm != NULL) {
		...
		// I. Fonts.
		{
			XRESOURCE_LOAD_STRING("italic", font_italic);
			XRESOURCE_LOAD_STRING("bold", font_bold);
			XRESOURCE_LOAD_STRING("roman", font_roman);
		}
		...
		XRESOURCE_LOAD_STRING("cursorfg", colorname[defaultcs])
			...
			XRESOURCE_LOAD_STRING("reverse-cursor", colorname[defaultrcs])
				...
				XRESOURCE_LOAD_INTEGER("padding", borderpx);
